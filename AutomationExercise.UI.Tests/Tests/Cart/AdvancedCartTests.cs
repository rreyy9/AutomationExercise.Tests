...
namespace AutomationExercise.UI.Tests.Tests.Cart
{
    ...
    public class AdvancedCartTests : BaseTest
    {
        ...
        // Steps 1-9 search and add to cart as guest.
        // Steps 10-12 log in and verify cart persists across login.
        ...
        {
            var faker = new Faker();
            _accountEmail = $"pw.tc20.{Guid.NewGuid():N}@example.com";
            ...
            var cartCountBefore = await cartPage.GetItemCountAsync();
            Assert.IsTrue(cartCountBefore > 0, "Expected at least one item in cart after adding search result");

            var productNameBefore = await cartPage.GetProductNameAsync(0);
            Log.Info($"Product in cart before login: '{productNameBefore}'");
            ...
            Log.Info($"Cart items after login: {cartCountAfterLogin}");
        }
